...
_BROWSER_HINTS = re.compile(
    ...
    r"\bdownload\b|"
    r"\bupload\b|"
    r"\btest\s+(?:each\s+)?(?:cases?|scenarios?)\b|"
    r"\b(?:cover|run|execute)\s+(?:all\s+)?(?:the\s+)?(?:test\s+)?(?:cases?|scenarios?)\b|"
    r"\b(?:verify|validate)\s+(?:all\s+)?(?:the\s+)?(?:test\s+)?(?:cases?|scenarios?)\b|"
    r"\b(?:test|testing)\s+(?:the\s+)?(?:chat\s+)?(?:app|application|ui)\b|"
    r"\bplaywright\b|"
